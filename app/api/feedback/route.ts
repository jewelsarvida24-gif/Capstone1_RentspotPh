...
export async function POST(request: Request) {
  ...
  const { data: booking } = await supabase.from('tbl_bookings').select('booking_status').eq('booking_id', booking_id).eq('user_id', userData.user.id).single();
  if (!booking || booking.booking_status !== 'completed') return NextResponse.json({ message: 'Only completed rentals can be rated.' }, { status: 400 });
