  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm sm:p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 pt-1 text-xs font-semibold text-neutral-800"><Clock3 className="h-4 w-4 text-blue-600" /> Real-Time Availability</div>
      </div>

      <div className="mt-4 rounded-xl border border-neutral-200 p-3">
        <div className="flex items-center justify-between"><button type="button" onClick={() => setVisibleMonth(subMonths(visibleMonth, 1))} className="rounded-lg p-1 text-neutral-700 hover:bg-neutral-100" aria-label="Previous month"><ArrowLeft className="h-4 w-4" /></button><p className="text-sm font-bold">{format(visibleMonth, 'MMMM yyyy')}</p><button type="button" onClick={() => setVisibleMonth(addMonths(visibleMonth, 1))} className="rounded-lg p-1 text-neutral-700 hover:bg-neutral-100" aria-label="Next month"><ArrowRight className="h-4 w-4" /></button></div>
